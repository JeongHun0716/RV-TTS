...
from parler_tts.configuration_parler_tts import ParlerTTSConfig, ParlerTTSDecoderConfig
...
class ParlerTTSDecoder(ParlerTTSPreTrainedModel):
    ...
    def __init__(self, config: ParlerTTSDecoderConfig):
        ...
        self.cat_embed = getattr(config, 'cat_embed', False)
        if self.cat_embed:
            assert (config.hidden_size % self.num_codebooks) == 0, 'Set hidden size d * 9'
            self.embed_tokens = nn.ModuleList(
                [nn.Embedding(embed_dim, config.hidden_size // self.num_codebooks) for _ in range(config.num_codebooks)]
            )
        else:
            self.embed_tokens = nn.ModuleList(
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if self.cat_embed:
                inputs_embeds = torch.cat([self.embed_tokens[codebook](input[:, codebook]) for codebook in range(num_codebooks)], -1)
            else:
...
class ParlerTTSForCausalLM(ParlerTTSPreTrainedModel):
    ...
    def forward(
        ...
        lm_logits = torch.stack([head(hidden_states) for head in self.lm_heads], dim=1) # B, 9, T, N_c
        ...
        if labels is not None:
            ...
            if not parallel:
                ...
                codebook_logits = rearrange(logits, 'b c t d -> (b c t) d')
                codebook_mask = rearrange(mask, 'b t c -> (b c t)')
                codebook_labels = rearrange(labels, 'b t c -> (b c t)')
    ...
    def generate(
        ...
        **kwargs,
        ...
        parallel = model_kwargs.get("parallel", False)
        if not parallel:
            input_ids, delay_pattern_mask = self.build_delay_pattern_mask(
                input_ids,
        ...
        if not parallel:
            ...
            output_ids = output_ids.reshape(batch_size, self.num_codebooks, -1)
...
class ParlerTTSForConditionalGeneration(PreTrainedModel):
    ...
    def __init__(
        ...
        self.cat_embed = getattr(self.config.decoder, 'cat_embed', False)
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
        prompt_spk_embedding: Optional[torch.FloatTensor] = None,
        parallel: Optional[bool] = False,
        **kwargs,
        ...
        if prompt_spk_embedding is not None:
            prompt_hidden_states = torch.cat([prompt_hidden_states, prompt_spk_embedding], dim=1)
            if prompt_attention_mask is not None:
                prompt_attention_mask = torch.cat([prompt_attention_mask, torch.ones_like(prompt_attention_mask)[:, :prompt_spk_embedding.size(1)]], dim=-1)
        ...
        if (labels is not None) and (decoder_input_ids is None and decoder_inputs_embeds is None):
            ...
            if self.config.decoder.dropout_input > 0 and self.training:
                keeping_ind = (decoder_input_ids == self.config.eos_token_id) + (decoder_input_ids == self.config.pad_token_id) + (decoder_input_ids == self.config.decoder_start_token_id)
                prob = torch.rand(decoder_input_ids.size())
                prob = prob < self.config.decoder.dropout_input
                dropout_input_ids = decoder_input_ids.clone()
                dropout_input_ids[prob] = self.config.decoder.mask_token_id
                dropout_input_ids[keeping_ind] = decoder_input_ids[keeping_ind]
                decoder_input_ids = dropout_input_ids
    ...
    def _prepare_prompt_kwargs_for_generation(self, prompt_input_ids, model_kwargs):
        prompt_hidden_states = self.embed_prompts(prompt_input_ids)
        prompt_spk_embedding = model_kwargs.get("prompt_spk_embedding", None)
        prompt_attention_mask = model_kwargs.get("prompt_attention_mask", None)
        if prompt_spk_embedding is not None:
            prompt_hidden_states = torch.cat([prompt_hidden_states, prompt_spk_embedding], dim=1)
            if prompt_attention_mask is not None:
                prompt_attention_mask = torch.cat([prompt_attention_mask, torch.ones_like(prompt_attention_mask)[:, :prompt_spk_embedding.size(1)]], dim=-1)
                model_kwargs["prompt_attention_mask"] = prompt_attention_mask
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif len(audio_scales) != batch_size:
            audio_scales = audio_scales * batch_size
